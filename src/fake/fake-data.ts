import { Client, Plan, Message } from "@/chat/interfaces/chat.interfaces";
...
export const checkAuth = async (token: string) => {
  await sleep(500);
  console.log("entro checkAuth");

  if (token !== "token-1234567890") {
    console.log("disparo el error");
...
const fakeMessages = {
  ...
  getMessages: (clientId: string) => {
    return fakeMessages.records[clientId] || [];
  },
  getAllMessages: () => {
...
export const getClientMessages = async (
  ...
  await sleep(300);
  //console.log(fakeMessages.getMessages(clientId))
...
//! message es de tipo Message, pero omitimos id y like
...
//! Creamos el objeto fakeMessages.records con los mensajes de los clientes
